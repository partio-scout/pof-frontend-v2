...
export const fetchComments = (suggestionId: number[], locale: string) => {
  const filters = suggestionId.map((id) => `filters[id][$in]=${id}`).join('&');
  return axios.get(
    `${trimUrl(
      process.env.GATSBY_API_URL,
    )}/api/suggestions?${filters}&populate[duration]=*&populate[locations][populate][icon]=*&populate[activity][populate][logo]=*&populate=*&locale=${
      locale || 'fi'
    }`,
  );
};
...
export const sendSuggestionLike = (
  suggestionId: number,
  userId: string,
): Promise<AxiosResponse<SuggestionFromRest | string>> => {
  return axios.post(`${trimUrl(process.env.GATSBY_API_URL)}/suggestions/${suggestionId}/like`, {
    user: userId,
  });
};

export const sendSuggestionUnlike = (
  suggestionId: number,
  userId: string,
): Promise<AxiosResponse<SuggestionFromRest | string>> => {
...
export const getActivityUrls = () => {
  ...
  const data = useStaticQuery(graphql`
    query ActivityUrlsQuery {
      activityUrls: allSitePage(filter: { context: { type: { eq: "Activity" } } }) {
        nodes {
