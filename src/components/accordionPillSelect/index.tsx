...
function AccordionPillSelect<T>({ title, ...props }: SelectProps<T>) {
  ...
  return (
    <div>
      <button
        className={clsx(
          'p-2 rounded-md border border-blue/40 hover:border-blue/100 flex justify-between cursor-pointer w-full font-bold',
          open ? 'bg-blue text-white rounded-b-none' : 'bg-white',
        )}
        onClick={() => setOpen(!open)}
      >
        {title}
        {open ? <MinusIcon className="fill-current text-white" /> : <PlusIcon className="fill-current/40" />}
      </button>
      <div
